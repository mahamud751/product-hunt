...
const MOCK_REWARDS: Reward[] = [
  {
    ...
    cashValue: 10,
    stock: 50,
    image: 'https://images.unsplash.com/photo-1622547748225-3fc4abd2cca0?auto=format&fit=crop&q=80&w=300&h=200',
  },
  {
    id: '2',
...
const RedeemModal = ({ isOpen, onClose, reward }: { isOpen: boolean; onClose: () => void; reward: Reward }) => {
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <div>
              ...
              <p className="text-[#AF583B] font-medium">{reward.pointsCost} Points</p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg space-y-2">
            ...
            <div className="border-t border-gray-200 pt-2 flex justify-between">
              ...
              <span className="font-medium">{2500 - reward.pointsCost} Points</span>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {pointsGuide.map((item) => (
              ...
              >
                ...
                <span className="font-medium text-[#AF583B]">+{item.points} Points</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card">
          ...
          <div className="space-y-4">
            {MOCK_ACTIVITIES.map((activity) => (
              ...
              >
                <div className="w-10 h-10 rounded-full bg-[#AF583B] bg-opacity-10 flex items-center justify-center">
                  {activity.type === 'upvote' && <ArrowUp className="w-5 h-5 text-[#AF583B]" />}
                  {activity.type === 'comment' && <MessageSquare className="w-5 h-5 text-[#AF583B]" />}
                  {activity.type === 'referral' && <UserPlus className="w-5 h-5 text-[#AF583B]" />}
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <p className="font-medium">{activity.description}</p>
                    <span className="text-[#AF583B] font-medium">+{activity.points} Points</span>
                  </div>
...
const Rewards = () => {
  ...
  const [sortBy, setSortBy] = useState<'cost' | 'popularity' | 'newest'>('cost');
  ...
  return (
    <div className="space-y-8">
      <div className="flex gap-4">
        ...
        <select
          value={sortBy}
          onChange={(e) => setSortBy(e.target.value as 'cost' | 'popularity' | 'newest')}
          className="border border-gray-200 rounded-lg px-4 py-2"
        >
      ...
      <div className="grid grid-cols-3 gap-6">
        {filteredRewards.map((reward) => (
          <div key={reward.id} className="card">
            ...
            <div className="flex items-center justify-between">
              <span className="text-[#AF583B] font-medium">{reward.pointsCost} Points</span>
              <button
                onClick={() => {
  ...
  return (
    <div className="space-y-8">
      <div className="flex gap-4">
        <select
          value={timeRange}
          onChange={(e) => setTimeRange(e.target.value as '7days' | '30days' | 'allTime')}
          className="border border-gray-200 rounded-lg px-4 py-2"
        >
        ...
        <div className="space-y-4">
          {MOCK_LEADERBOARD.map((user, index) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span className="text-2xl font-bold text-[#AF583B]">#{user.rank}</span>
              </div>
            </div>
          ))}
        </div>
...
export default function RewardsTab() {
  const [activeSection, setActiveSection] = useState<'dashboard' | 'activities' | 'rewards' | 'leaderboard'>('dashboard');
