...
export default function UserRewards({ dateFilter }: { dateFilter: string }) {
  ...
  const [sortBy, setSortBy] = useState<
    "userName" | "totalPoints" | "lastActivity"
  >("totalPoints");
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {user.totalPoints.toLocaleString()} total
                    </div>
                    <div className="text-sm text-gray-500">
                      {user.availablePoints.toLocaleString()} available
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getTierColor(
                        user.tier
                      )}`}
                    >
                      {user.tier}
