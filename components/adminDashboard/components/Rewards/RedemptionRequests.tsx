...
import { Search, Filter, MoreVertical, Gift, CheckCircle, XCircle, Clock, DollarSign, Download } from 'lucide-react';
...
export default function RedemptionRequests({ dateFilter }: { dateFilter: string }) {
  const [selectedRequests, setSelectedRequests] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [typeFilter, setTypeFilter] = useState('All');
  const [statusFilter, setStatusFilter] = useState('All');
  const [sortBy, setSortBy] = useState<'userName' | 'pointCost' | 'requestedAt'>('requestedAt');
  ...
  const filteredRequests = mockRedemptions
    .filter(request => 
      (request.userName.toLowerCase().includes(searchQuery.toLowerCase()) ||
       ...
       request.reward.name.toLowerCase().includes(searchQuery.toLowerCase())) &&
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-[#1F1F1F]">Redemption Requests</h2>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredRequests.map((request) => (
                <tr 
                  key={request.id} 
                  className={`hover:bg-gray-50 cursor-pointer ${selectedRequestId === request.id ? 'bg-gray-50' : ''}`}
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{request.reward.name}</div>
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getRewardTypeColor(request.reward.type)}`}>
                      {request.reward.type}
                    </span>
                  </td>
          ...
          {selectedRequest ? (
            <>
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-[#1F1F1F]">Request Details</h3>
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-medium text-gray-900">{selectedRequest.reward.name}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Value</span>
                      <span className="text-sm font-medium text-gray-900">${selectedRequest.reward.value}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Points Cost</span>
                      <span className="text-sm font-medium text-gray-900">{selectedRequest.reward.pointCost.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedRequest.updatedAt !== selectedRequest.requestedAt && (
                      <div className="flex items-start space-x-3">
                        <div className="flex-shrink-0">
                          <Clock className="w-4 h-4 text-gray-400" />
                        </div>
