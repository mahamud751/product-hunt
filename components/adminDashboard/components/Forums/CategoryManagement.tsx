...
import { Search, Filter, MoreVertical, Plus, Edit3, Trash2, MessageSquare } from 'lucide-react';
...
export default function CategoryManagement({ dateFilter }: { dateFilter: string }) {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'threadCount' | 'createdAt'>('createdAt');
  ...
  const filteredCategories = mockCategories
    .filter(category => 
      ...
      (visibilityFilter === 'All' || 
       (visibilityFilter === 'Private' ? category.isPrivate : !category.isPrivate))
    )
    .sort((a, b) => {
      if (sortBy === 'threadCount') {
        return sortOrder === 'asc' ? a.threadCount - b.threadCount : b.threadCount - a.threadCount;
      }
              ...
              <tr key={category.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                    category.isPrivate ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'
                  }`}>
