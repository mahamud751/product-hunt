...
import { Search, Filter, MoreVertical, Flag, Ban, CheckCircle, XCircle, MessageSquare, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold text-[#1F1F1F]">Flagged Content</h2>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredThreads.map((thread) => (
              <tr key={thread.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getReasonColor(thread.flagReason)}`}>
                    {thread.flagReason}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    <div className="flex-shrink-0 h-8 w-8">
                      <img className="h-8 w-8 rounded-full" src={thread.reportedBy.avatar} alt="" />
                    </div>
                    <div className="ml-3">
                      <div className="text-sm font-medium text-gray-900">{thread.reportedBy.name}</div>
                      <div className="text-sm text-gray-500">{thread.reportedBy.email}</div>
                    </div>
                  </div>
                </td>
