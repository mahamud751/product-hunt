...
export default function UserHistory({ dateFilter }: { dateFilter: string }) {
  ...
  const [sortBy, setSortBy] = useState<'userName' | 'totalUpvotes' | 'lastActivity'>('lastActivity');
  ...
  const filteredUsers = mockUserHistory
    ...
    .sort((a, b) => {
      if (sortBy === 'totalUpvotes') {
        return sortOrder === 'asc' ? a.totalUpvotes - b.totalUpvotes : b.totalUpvotes - a.totalUpvotes;
      }
